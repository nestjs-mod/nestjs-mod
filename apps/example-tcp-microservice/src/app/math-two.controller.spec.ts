...
describe('Math over TCP with two servers', () => {
  ...
  beforeAll(async () => {
    ...
    server1 = await bootstrapNestApplication({
      ...
      project: {
        name: 'TestTwoMicroserviceServer',
        description: 'Test microservice server',
      },
      modules: {
        system: [
          TcpNestMicroservice.forRoot({ staticEnvironments: { port: 5010, host: 'localhost' } }),
    ...
    server2 = await bootstrapNestApplication({
      // logger: new Logger('Server'),
      project: {
        name: 'TestTwoMicroserviceServer2',
        description: 'Test microservice server 2',
      },
      modules: {
        system: [
          TcpNestMicroservice.forRoot({ staticEnvironments: { port: 5012, host: 'localhost' } }),
          DefaultNestApplicationListener.forRoot(),
        ],
        feature: [AppModule.forRoot()],
      },
    ...
    client = await bootstrapNestApplication({
      ...
      project: {
        name: 'TestTwoMicroserviceClient',
          ...
          createNestModule({
            ...
            imports: [
              TcpNestMicroserviceClientModule.forRoot({
                contextName: 'ms1',
                staticConfiguration: { microserviceProjectName: 'TestTwoMicroserviceServer' },
                staticEnvironments: {
                  port: 5010,
                  host: 'localhost',
                },
              ...
              TcpNestMicroserviceClientModule.forRoot({
                contextName: 'ms2',
                staticConfiguration: { microserviceProjectName: 'TestTwoMicroserviceServer2' },
                staticEnvironments: { port: 5012, host: 'localhost' },
              }),
            ],
