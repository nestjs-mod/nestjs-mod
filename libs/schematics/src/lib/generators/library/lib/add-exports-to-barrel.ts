...
export function addExportsToBarrelFile(tree: Tree, options: NormalizedOptions): void {
  if (!tsModule) {
  ...
  if (exportStatement) {
    sourceFile = removeChange(tree, sourceFile, indexPath, 0, exportStatement.getFullText());
  }
  sourceFile = addGlobal(tree, sourceFile, indexPath, `export * from './lib/${options.fileName}.module';`);

  if (options.service) {
    sourceFile = addGlobal(tree, sourceFile, indexPath, `export * from './lib/${options.fileName}.service';`);
  }
  ...
  sourceFile = addGlobal(tree, sourceFile, indexPath, `export * from './lib/${options.fileName}.constants';`);
}
