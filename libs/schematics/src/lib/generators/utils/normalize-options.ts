...
import type { Language, NestGeneratorWithLanguageOption, NormalizedOptions, UnitTestRunner } from './types';
...
export async function normalizeOptions(
  ...
  const { directory, artifactName, fileExtension } = await determineArtifactNameAndDirectoryOptions(tree, {
    path: options.path,
  ...
  options.path = ''; // Now that we have `directory` we don't need `path`
...
export function unitTestRunnerToSpec(unitTestRunner: UnitTestRunner | undefined): boolean | undefined {
